...
| LLM07 | 2    | Config             | As a user, I want the AI's sensitivity to be easily adjustable, so its behavior can be fine-tuned based on preference. | `LLM_ENDPOINT` loaded from env; `SIM_STRONG`/`SIM_WEAK` thresholds configurable; validation on load; changes take effect after restart.                                        | 5      | 6    | 60%    | Mann  |
